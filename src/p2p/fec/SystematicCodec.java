...
public class SystematicCodec {
    ...
    private void generateMatrix(int k, int n) {
        ...
        for (int row = 0; row < n - 1; row++) {
            for (int col = 0; col < k; col++) {
                tmp[row + 1][col] = GFMath.gfExp(GFMath.gfMod(row * col));
            }
        }
    ...
    private short[] addConstantMultiple(short[] destRow, short[] srcRow,
        ...
        for (int i = 0; i < destRow.length; i++) {
            destRow[i] ^= GFMath.gfMultiply(constant, srcRow[i]);
        }
    ...
    private short[][] matrixMultiply(short[][] a, short[][] b) {
        ...
        for (int row = 0; row < n; row++) {
            for (int col = 0; col < m; col++) {
                for (int i = 0; i < k; i++) {
                    c[row][col] ^= 
                        GFMath.gfMultiply(a[row][i], b[i][col]);
                }
            }
        }
        return c;
    ...
    private short[][] invertMatrix(short[][] a) {
        ...
        for (int col = 0; col < k; col++) {
            ...
            if (c != 1) {
                c = GFMath.gfInverse(c);
                pivotRow[icol] = 1;
                for (int i = 0; i < k; i++) {
                    pivotRow[i] = GFMath.gfMultiply(c, pivotRow[i]);
                }
            }
    ...
    private short[][] invertVandermonde(short[][] a) {
        ...
        for (int i = 1; i < k; i++) {
            ...
            for (int j = (k - 1) - (i - 1); j < k - 1; j++) {
                c[j] ^= GFMath.gfMultiply(p_i, c[j + 1]);
            }
            c[k - 1] ^= p_i;
        }
        ...
        for (int row = 0; row < k; row++) {
            ...
            b[k - 1] = 1;
            for (int i = k - 2; i >= 0; i--) {
                b[i] = (short)(c[i + 1] ^ GFMath.gfMultiply(xx, b[i + 1]));
                t = (short)(GFMath.gfMultiply(xx, t) ^ b[i]);
            }
            
            for (int col = 0; col < k; col++) {
                a[row][col] = GFMath.gfMultiply(GFMath.gfInverse(t), b[col]);
            }
        }
        return a;
    }
